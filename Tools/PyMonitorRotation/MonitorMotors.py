...
if monitorFromLog:
    ...
    motor1 = MotorRotation(False, logReader, 0, 3, 9600, 100000)
    motor2 = MotorRotation(False, logReader, 1, 3, 9600, 100000)
else:
    serial2 = serial.Serial(serialPort2, baudRate)
    motor1 = MotorRotation(True, serial1, 0, 3, 9600, 100000)
    motor2 = MotorRotation(True, serial2, 1, 3, 9600, 100000)
motor1.startReader()
motor2.startReader()
...
def calcXY(angle1, angle2):
    ang1 = angle1 * math.pi / 180
    ang2 = (180 - angle2) * math.pi / 180
    l1 = 92.5
    l2 = 92.5
    elbowX = math.sin(ang1) * l1
    elbowY = math.cos(ang1) * l1
...
if USE_MATPLOTLIB:
    ...
    def drawNext():
        ...
        meas1 = motor1.getNewMeasurements()
        meas2 = motor2.getNewMeasurements()
        for i in range(min(len(meas1),len(meas2))):
            x,y = calcXY(meas1[i][1], meas2[i][1])
            # line = canvas.create_line(curX,curY,x,y,fill="red")
            canvas.create_line(0,0,100,100+y,fill="red")
            print("H")
            curX = x
            curY = y
        canvas.after(100,drawNext)
