...
static int JSMNR_parse_primitive(JSMNR_parser *parser, const char *js,
	...
	for (; parser->pos < len && js[parser->pos] != '\0'; parser->pos++) {
		...
		if (js[parser->pos] < 32 || js[parser->pos] >= 127) {
            Log.trace(F("JSMNR_ERROR_INVAL ch bounds %d pos %d"CR), js[parser->pos], parser->pos);
            parser->pos = start;
	...
	for (; parser->pos < len && js[parser->pos] != '\0'; parser->pos++) {
		...
		if (c == '\\' && parser->pos + 1 < len) {
			...
			switch (js[parser->pos]) {
				...
				case 'u':
					...
					for(int i = 0; i < 4 && parser->pos < len && js[parser->pos] != '\0'; i++) {
						...
						if(!((js[parser->pos] >= 48 && js[parser->pos] <= 57) || // 0-9
                            ...
                            Log.trace(F("JSMNR_ERROR_INVAL hex bounds %d pos %d"CR), js[parser->pos], parser->pos);
							parser->pos = start;
							return JSMNR_ERROR_INVAL;
						}
						parser->pos++;
				...
				// Unexpected symbol
				default:
                    Log.trace(F("JSMNR_ERROR_INVAL Unexpected %d pos %d"CR), js[parser->pos], parser->pos);
					parser->pos = start;
					return JSMNR_ERROR_INVAL;
			}
		}
		...
		switch (c) {
            ...
            case ']':
				...
				for (i = parser->toknext - 1; i >= 0; i--) {
					...
					if (token->start != -1 && token->end == -1) {
						if (token->type != type) {
                            Log.notice(F("JSMNR_ERROR_INVAL %d type %d %d"CR), token->start, token->type, type);
							return JSMNR_ERROR_INVAL;
						}
						parser->toksuper = -1;
						token->end = parser->pos + 1;
				...
				if (i == -1)
                {
                    Log.notice(F("JSMNR_ERROR_INVAL unmatchedbrace pos %d ch %d toknext %d type %d"CR), parser->pos, js[parser->pos], parser->toknext, type);
