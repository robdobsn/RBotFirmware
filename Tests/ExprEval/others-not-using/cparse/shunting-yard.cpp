...
bool rpnBuilder::handle_unary(const std::string& op) {
  if (this->lastTokenWasOp) {
    ...
    if (!op.compare("-") || !op.compare("+")) {
      this->rpn.push(new Token<int64_t>(0, INT_TOKTYPE));
  ...
  while (*expr && (data.bracketLevel || !strchr(delim, *expr))) {
    if (isdigit(*expr)) {
      ...
      if (!strchr(".eE", *nextChar)) {
        data.handle_token(new Token<int64_t>(_int, INT_TOKTYPE));
      } else {
      ...
      if (*expr != quote) {
        ...
        rpnBuilder::cleanRPN(&data.rpn);
        // throw syntax_error("Expected quote (" + squote + ") at end of string declaration: " + squote + ss.str() + ".");
      }
      ++expr;
      ...
      if (data.lastTokenWasUnary) {
        ...
        op.push_back(*expr);
        rpnBuilder::cleanRPN(&data.rpn);
        // throw syntax_error("Expected operand after unary operator `" + data.opStack.top() + "` but found: `" + op + "` instead.");
      }
