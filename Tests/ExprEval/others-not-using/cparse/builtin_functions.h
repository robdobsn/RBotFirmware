...
packToken default_type(TokenMap scope) {
  ...
  case INT_TOKTYPE: return "integer";
...
const args_t minmax_args = {"num1", "num2"};
packToken default_min(TokenMap scope) {
  // Get both arguments:
  double number1 = scope["num1"].asDouble();
  double number2 = scope["num2"].asDouble();
...
struct Startup {
  Startup() {
    ...
    global["min"] = CppFunction(&default_min, minmax_args, "min");
    global["max"] = CppFunction(&default_max, minmax_args, "max");
