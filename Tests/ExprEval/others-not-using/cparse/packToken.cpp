...
static std::string emptyStdString;
static TokenMap emptyTokenMap;
static TokenList emptyTokenList;
static Tuple emptyTuple;
static STuple emptySTuple;
static CppFunction emptyFunction(NULL, "empty");
...
packToken& packToken::operator[](const std::string& key) {
  if (base->type != MAP) {
    return (packToken&)packToken::None;
//    throw bad_cast("The Token is not a map!");
  }
  return (*static_cast<TokenMap*>(base))[key];
}
const packToken& packToken::operator[](const std::string& key) const {
  if (base->type != MAP) {
    return (const packToken&)packToken::None;
    //throw bad_cast("The Token is not a map!");
  }
  return (*static_cast<TokenMap*>(base))[key];
}
...
double packToken::asDouble() const {
  if (!(base->type & NUM)) {
    return 0;
    // throw bad_cast("The Token is not a number!");
  }
  if (base->type == REAL) {
...
std::string& packToken::asString() const {
  if (base->type != STR && base->type != VAR && base->type != OP) {
    ...
    // throw bad_cast("The Token is not a string!");
  }
...
//    throw bad_cast("The Token is not a list!");
  }
...
Tuple& packToken::asTuple() const {
  if (base->type != TUPLE) {
    return emptyTuple;
    // throw bad_cast("The Token is not a tuple!");
  }
...
STuple& packToken::asSTuple() const {
  if (base->type != STUPLE) {
    return emptySTuple;
    // throw bad_cast("The Token is not an special tuple!");
  }
...
Function* packToken::asFunc() const {
  if (base->type != FUNC) {
    ...
    // throw bad_cast("The Token is not a function!");
  }
