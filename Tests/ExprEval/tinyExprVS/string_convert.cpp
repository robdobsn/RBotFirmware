...
char* spark_ultoa(unsigned long a, char* buffer, int radix, char pad) {
	if (radix<2 || radix>36) {
		return NULL;
	}
	char* ptr = buffer;
...
char* spark_itoa(int a, char* buffer, int radix) {
	if (a<0) {
		*buffer = '-';
		...
		spark_ultoa((unsigned)v, buffer + 1, radix);
	}
	else {
		spark_ultoa(a, buffer, radix);
	}
	return buffer;
}

char* spark_utoa(unsigned a, char* buffer, int radix) {
	return spark_ultoa(a, buffer, radix);
}
