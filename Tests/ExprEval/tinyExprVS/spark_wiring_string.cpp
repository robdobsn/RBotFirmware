...
void dtoa(double val, unsigned char prec, char *sout) {
	...
	spark_ultoa(first, sout, 10, 1);
	if (prec) {
		sout += strlen(sout);
		*sout++ = '.';
		spark_ultoa(second, sout, 10, prec);
	}
}
