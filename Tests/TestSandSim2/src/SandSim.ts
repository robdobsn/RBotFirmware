...
interface SandDisplacement
{
    level: number;
    sourcePt: Point;
    dests: Array<SandDest>;
}

class SandKernel 
{
    displacementKernel:Array<SandDisplacement> = [];

    constructor(numRings:number, ringSpacing:number, ballDiameter:number)
    {
        // Create bin-rings that are a list of points in a square grid
        // that are at various distances from the centre of the grid
        let binRings:Array<Array<Point>> = this.createBinRings(numRings, ringSpacing);
    ...
    createBinRings(numRings:number, ringSpacing:number):Array<Array<Point>>
    {
        ...
        let binRings:Array<Array<Point>> = [];
        for (let i:number = 0; i < numRings; i++)
            binRings.push([])
        const gridCentre:Point = {x:0,y:0};
        for (let x:number = -numRings; x <= numRings; x++)
        {
            for (let y:number = -numRings; y <= numRings; y++)
            {
                let thisPt:Point = {x:x,y:y};
                let ptDist:number = Math.floor(this.distFrom(thisPt, gridCentre));
    ...
    displacementLevel(ringIdx:number, ringSpacing:number, ballDiameter:number):number
    {
        let ringDist:number = ringIdx * ringSpacing;
        let level:number = 0;
    ...
    r2d(rads:number):number
    {
        return rads * 180 / Math.PI;
    }

    calcAngleBetweenPointVecsDegs(pt1:Point, pt2:Point):number
    {
        // Calculate angle subtended at origin (0,0) between two points
        let a1:number = Math.atan2(pt1.x, pt1.y);
        let a2:number = Math.atan2(pt2.x, pt2.y);
        if (a1 > a2)
            return this.r2d(a1-a2);
        return this.r2d(a2-a1);
    }

    getSandDests(binRings:Array<Array<Point>>, srcRingIdx:number, srcPoint:Point, destAngleRangeDegs:number):Array<SandDest>
    {
        // To return
        let destPtAndWts:Array<SandDest> = [];
        // Accumulate angle weights
        let angleWeightAccum:number = 0;
        // Iterate all possible points in the next available ring outwards from the source point's ring
        for (let destPtIdx:number = 0; destPtIdx < binRings[srcRingIdx+1].length; destPtIdx++)
        {
            // Check if point is within range
            let destPt:Point = binRings[srcRingIdx+1][destPtIdx];
            let vecAngleDegs:number = this.calcAngleBetweenPointVecsDegs(srcPoint, destPt);
            if (vecAngleDegs < destAngleRangeDegs/2)
            {
                // Calculate angle weight
                let angleWeight:number = destAngleRangeDegs/2 - vecAngleDegs;
        ...
        for (let sandDestIdx:number = 0; sandDestIdx < destPtAndWts.length; sandDestIdx++)
        {
    ...
    createDisplacementKernel(binRings:Array<Array<Point>>, 
                ringSpacing:number, ballDiameter:number):Array<SandDisplacement>
    {
        ...
        let displacementKernel:Array<SandDisplacement> = [];
        for (let ringIdx:number = 0; ringIdx < binRings.length-1; ringIdx++)
        {
            ...
            for (let j:number = 0; j < binRings[ringIdx].length; j++)
            {
                ...
                let sandDests:Array<SandDest> = 
                            this.getSandDests(binRings, ringIdx, binRings[ringIdx][j], destAngleRangeDegs);
                // Dest info
                let sandDisp:SandDisplacement = {
                                level:dispLevel, 
    ...
    distFrom(pt1:Point, pt2:Point):number
    {
        return Math.sqrt(Math.pow(pt2.x-pt1.x,2)+Math.pow(pt2.y-pt1.y,2));
    }

    moveSand(sandMatrix:Array<Array<number>>, pt:Point, angleDegs:number) {
        // Iterate source points
        for (let i:number = 0; i < this.displacementKernel.length; i++)
        {
            // Displacement
            let sandDisp:SandDisplacement = this.displacementKernel[i];
            ...
            if ((srcY >= 0) && (srcY < sandMatrix.length) && (srcX >= 0) && (srcX < sandMatrix[0].length))
            {
                ...
                {
                    let sandToDisperse:number = sandMatrix[srcY][srcX] - sandDisp.level;
                    sandMatrix[srcY][srcX] = sandDisp.level;
                    
                    // Disperse sand
                    let sandRemaining:number = sandToDisperse;
                    for (let j:number = 0; j < sandDisp.dests.length; j++)
                    {
                        // Check validity of destination
                        let dest:SandDest = sandDisp.dests[j];
                        let destX:number = dest.destPt.x + pt.x;
                        let destY:number = dest.destPt.y + pt.y;
                        if ((destY >= 0) && (destY < sandMatrix.length) && (destX >= 0) && (destX < sandMatrix[0].length))
                        {
                            let moveAmount:number = sandToDisperse * dest.weight;
...
class SandSim {
    ...
    // Vars
    sandTableSideLen: number;
    kernelSize: number;
    ringSpacing: number;
    sandMatrix: Array<Array<number>>;
    ballPos: Point;
    ballDiameter: number;
    sandStartLevel: number;
    maxSandLevel: number;
    dirtyPointList: Array<Point>;
    sandKernel: SandKernel;

    constructor(ballDiameter: number, sandTableSideLen: number)
    {
