...
export function getPatternPoint(
  ...
  switch(pattern) {
    ...
    case 'spiral': // Archimedean Spiral
      if (t < 2 * Math.PI * 410) {
        x = Math.sin(t/20) * (230 - t/5);
        y = Math.cos(t/20) * (230 - t/5);
      } else {
        x = Math.sin((t-15)/20) * 250;
        y = Math.cos((t-15)/20) * 250;
      }
      break;
    ...
    case 'fermatSpiral': // Fermat's Spiral
      const angle = t / 15;
      const radius = Math.min(15 * Math.sqrt(angle), maxRadius);
    ...
    case 'rose': // Rose Curve
      ...
      const r_rose = maxRadius * 0.8 * Math.cos(n / d * theta_rose);
    ...
    case 'epitrochoid': // Epitrochoid
      const R = 100; // fixed circle radius
      const r_epi = 40; // rolling circle radius
      const d_epi = 80; // distance from center of rolling circle
    ...
    case 'hypotrochoid': // Hypotrochoid
      const R_hypo = 150; // fixed circle radius
      const r_hypo = 50; // rolling circle radius
      const d_hypo = 80; // distance from center of rolling circle
    ...
    case 'circles': // Expanding Circles
      ...
      const circleR = Math.min(30 + circleNum * 30, maxRadius);
    ...
    case 'spiral3d': // 3D-looking Spiral
      const theta_3d = t / 20;
      const r_3d = Math.min(10 + theta_3d * 3, maxRadius);
